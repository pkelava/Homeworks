...
function domPath() {
  ...
  mainDiv.onclick = function (event) {
    if (pathToFirstElem.length === 0) {
      ...
      pathToFirstElem = event.path.reverse(); //izbacujemo node-ove koji se nalaze van body-a

      pathToFirstElem = pathToFirstElem.slice(3, pathToFirstElem.length);
    } else {
      pathToSecondElem = event.path.reverse();
      pathToSecondElem = pathToSecondElem.slice(3, pathToSecondElem.length); //Imamo put do prvog elementa i put do drugog elementa, jer oba imaju "body" kao root
